...
function main(): void {
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    // 10), and v2's pan/zoom relies on being able to do it.
    ...
    if (drag.kind === 'state') {
      ...
      // keeps its relative layout (rather than snapping each object). Round
      // even without grid snap - x0/y0/x/y are all ints in Java, and rounding
      // the shared delta once (not each object's origin+delta) keeps every
      // member's relative offset exact.
      if (doc.preferences.grid) {
